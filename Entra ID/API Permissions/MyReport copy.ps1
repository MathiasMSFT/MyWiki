...
Param(
    ...
    [switch]$IncludeOwnerOrg=$false,
    [switch]$IncludeCSA=$false,
    [switch]$IncludeSignInStats=$false,
...
function parse-CustomSecurityAttributes {
    Param([Parameter(Mandatory=$true)]$customSecurityAttributes)
    $out = @();
...
$tenantId = "identityms.onmicrosoft.com" # Your tenant ID or domain
